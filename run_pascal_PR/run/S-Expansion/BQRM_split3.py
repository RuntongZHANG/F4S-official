...
from data.dataset import FSSDataset_WildImage

os.environ["CUDA_VISIBLE_DEVICES"] = '1'

def test(model, model_aux, dataloader, nshot):
    r""" Test HSNet """

    # Freeze randomness during testing for reproducibility
    utils.fix_randseed(0)

    class_list = [15,16,17,18,19]
    for class_ in class_list:
        dataloader.dataset.class_sample=class_
        ...
        for idx, batch in enumerate(dataloader):
            ...
            if QS > 0.3:
                ...
                Image.fromarray(np.uint8(pseudo_label)).convert('L').save(
                    '/media/meng2/disk11/ZRT/PycharmProjects/Datasets_HSN/VOC2012/S-Expansion/split3/Buffer/%s_class%d.png'
        ...
        with open('/media/meng2/disk11/ZRT/PycharmProjects/Datasets_HSN/VOC2012/S-Expansion/split3/pascal_split3_class%d.pkl'%(class_),'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--benchmark', type=str, default='pascal_wild', choices=['pascal', 'coco', 'fss'])
    parser.add_argument('--logpath', type=str, default='exp1')
    parser.add_argument('--bsz', type=int, default=1)
    parser.add_argument('--nworker', type=int, default=0)
    parser.add_argument('--load', type=str, default='checkpoints/pascal/res101_pas/res101_pas_fold3/best_model.pt')
    parser.add_argument('--load_aux', type=str, default='checkpoints/pascal/res50_pas/res50_pas_fold3/best_model.pt')
    parser.add_argument('--fold', type=int, default=3, choices=[0, 1, 2, 3])
    parser.add_argument('--nshot', type=int, default=1)
    parser.add_argument('--nexpansion', type=int, default=0)
    ...
    FSSDataset_WildImage.initialize(img_size=400, datapath=args.datapath, use_original_imgsize=args.use_original_imgsize)
    dataloader_test = FSSDataset_WildImage.build_dataloader(args.benchmark, args.bsz, args.nworker, args.fold, 'test', args.nshot, args.nexpansion)

    # Test HSNet
    with torch.no_grad():
        test(model, model_aux, dataloader_test, args.nshot+args.nexpansion)
