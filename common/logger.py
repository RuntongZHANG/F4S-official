...
class AverageMeter:
    ...
    def write_result(self, split, epoch):
        iou, fb_iou, iou_bg, miou_fg_list = self.compute_iou()
        ...
        msg += 'mIoU_bg: %5.2f   '% iou_bg
        for i in range(len(miou_fg_list)):
            msg += 'Class%d: %5.2f   ' % (i, miou_fg_list[i])
    ...
    def write_process(self, batch_idx, datalen, epoch, write_batch_idx=20):
        if batch_idx % write_batch_idx == 0:
            ...
            iou, fb_iou, _, _ = self.compute_iou()
            if epoch != -1:
                loss_buf = torch.stack(self.loss_buf)
